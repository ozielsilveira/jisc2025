...
import { CheckCircle, FileText, Share2, Users, X, Search, Filter, Mail, Phone, Calendar, Trophy, MapPin, Eye, UserCheck, UserX, Download, Grid3X3, List, SortAsc, SortDesc, Loader2, AlertCircle, Star } from 'lucide-react'
...
const AthleteAvatar = ({ athlete }: { athlete: Athlete }) => {
  ...
  return (
    <Avatar className="h-12 w-12">
      <AvatarImage 
        src={`/placeholder-icon.png?height=48&width=48&text=${initials}`} 
...
export default function AthletesPage() {
  ...
  const { athletes, setAthletes, athletics, sports, isLoading, athleticId, refetch } = useAthletes(user, userRole)
  ...
  const filteredAndSortedAthletes = useMemo(() => {
    let filtered = athletes.filter((athlete) => {
      ...
      const matchesSport = selectedSportFilter === 'all' || 
                          athlete.sports.some(sport => sport.id === selectedSportFilter)
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between text-sm text-gray-500">
                  <span>
                    {filteredAndSortedAthletes.length} atleta{filteredAndSortedAthletes.length !== 1 ? 's' : ''} encontrado{filteredAndSortedAthletes.length !== 1 ? 's' : ''}
                  </span>
                  
                  {(searchTerm || selectedAthleticFilter !== 'all' || selectedStatusFilter !== 'all' || selectedSportFilter !== 'all') && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => {
